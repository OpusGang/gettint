...
def gettint(src: str,
    ...
    backup_tests = {
            ...
            "offset+gain" : lambda c: _offset_gain(c, [round(vals[0], 3) for vals in match_res[1]], [round(vals[1], 3)
                for vals in match_res[1]]),
            "levels"      : lambda c: _levels(c, match_res[2])
            }
    ...
    match_tests = ["gamma", ["gain", "offset"], "levels"]
    match_fmts = [vs.YUV444P8, vs.RGB24, vs.YUV444P8]
    match_res = []
...
def main():
    ...
    gettint(args.src, args.ref, args.frame)
